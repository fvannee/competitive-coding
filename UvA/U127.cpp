...
int main() {
   ...
   while (!stop)
   {
      ...
      if (stop)
         break;*/
      bool cont = true;
      for (int jj = 0; jj < 3000 && cont; jj++)
      {
         cont = false;
         int firstI = 1;
         REP(i, firstI, cards.size())
         {
            if (i >= 3 && matches(cards[i].top(), cards[i-3].top()))
            {
            ...
            if (i >= 1 && matches(cards[i].top(), cards[i - 1].top()))
            {
