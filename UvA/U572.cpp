...
int indexof(int x, int y, int size)
{
   return x * size + y;
}

void unionOil(vector<string> &oil, int oldX, int oldY, int x, int y, int size)
{
   if (x >= 0 && y >= 0 && y < size && oil[x][y] == '@')
   {
      unionSet(indexof(oldX, oldY, size), indexof(x, y, size));
   }
}
   ...
   while (true)
   {
      ...
      FORN(i, x) FORN(j, y)
      {
         if (oil[i][j] == '@')
         {
            ...
            unionOil(oil, i, j, i - 1, j - 1, y);
            unionOil(oil, i, j, i - 1, j, y);
            unionOil(oil, i, j, i - 1, j + 1, y);
            unionOil(oil, i, j, i, j + 1, y);
            unionOil(oil, i, j, i, j - 1, y);
         }
      }
      int dj = numDisjointSets();
      int sets = dj + numoils - total;
      cout << sets << endl;
   }
}
