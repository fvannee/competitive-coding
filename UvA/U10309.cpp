...
void toggle_one(bool lights[][10], int i, int j)
{
   if (i >= 0 && i < 10 && j >= 0 && j < 10)
      lights[i][j] = !lights[i][j];
}

void toggle(bool lights[][10], int i, int j)
{
   toggle_one(lights, i, j);
   toggle_one(lights, i - 1, j);
   toggle_one(lights, i + 1, j);
   toggle_one(lights, i, j - 1);
   toggle_one(lights, i, j + 1);
}
...
int main() {
   while (true)
   {
      ...
      memcpy(copy, lights, sizeof(lights));
      int maxi = 101;
      FORN(mask, 1 << 10)
      {
         memcpy(lights, copy, sizeof(lights));
         int cnt = 0;
         FORN(i, 10)
         {
            if ((mask & (1 << i)) != 0)
