...
int main() {
   while (true)
   {
      ...
      for (int i = pancakes.size() - 1; i >= 0; i--)
      {
         for (int j = i; j >= 0; j--)
         {
            if (pancakes[i] == pancakes2[j])
            {
               if (i == j)
                  break;
               if (j != 0)
               {
                  reverse(pancakes2.begin(), pancakes2.begin() + j + 1);
                  cout << pancakes.size() - j << " ";
               }
               cout << pancakes.size() - i << " ";
               
               reverse(pancakes2.begin(), pancakes2.begin() + i + 1);
               break;
            }
         }
      }
