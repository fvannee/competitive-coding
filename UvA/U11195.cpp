...
inline bool canplace(int r, int c, int m)
{
   return (rows & (1 << c)) == 0 && (dl & (1 << (r + c))) == 0 && (dr & (1 << (m - r + c))) == 0;
   //return rows[c] == -1 && dl[r+c] == -1 && dr[m-r+c] == -1;
}
...
void solve(int n, int row, int dl, int dr)
{
   if (row == mask)
   {
      total++;
   ...
   int m = mask & bad2[n] & ~(row | dl | dr);
   while (m != 0)
   {
      int b = m & (-m);
      m -= b;
      solve(n + 1, row | b, (dl | b) << 1, (dr | b) >> 1);
   }
}
