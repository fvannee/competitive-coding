...
ll findlength(ll n)
{
   ...
   {
      first = last + cnt;
      last = first + (mod - prevmod - 1) * cnt;
      sum += (mod - prevmod) * (first + last) / 2;
      cnt++;
      
      prevmod = mod;
      mod *= 10;
   }
   int rest = n - prevmod + 1;
   first = last + cnt;
   last = first + (rest - 1) * cnt;
   sum += rest * (first + last) / 2;
   return sum;
}
...
int main() {
   ...
   FORN(kk, ntc)
   {
      ...
      ll low = 1, high = 2147483647;
      ll bestans = 0;
      while (low < high)
      {
         ll mid = (low + high) / 2;
         ll ans = findlength(mid);
         if (ans <= n)
         {
            low = mid + 1;
            bestans = ans;
