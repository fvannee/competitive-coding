...
bool solve(vi &inp, int curnum, int curinp, int burned)
{
   if (curinp >= inp.size())
      return true;
   int n = nums[curnum] & ~burned;
   if ((n | inp[curinp]) == n)
      return solve(inp, curnum - 1, curinp + 1, (n ^ inp[curinp]) | burned);
   else
      return false;
}
...
int main() {
   while (true)
   {
      ...
      REP(i, n - 1, 10)
      {
         if ((nums[i] | inp[0]) == nums[i])
         {
            ans = solve(inp, i - 1, 1, nums[i] ^ inp[0]);
            if (ans)
               break;
         }
      }
